...
module.exports = {
  apps: [
    ...
    },
    {
      ...
      cwd: '${APP_HOME}/${DEPLOY_SERVICE_NAME}',
      script: 'activator.js',
      // pm2 gives up after max_restarts (15) by default, which is exactly
      // wrong for a startup crash — the two real failures so far were a
      // parse error and a require-time throw, both of which would have
      // left this down permanently. Backoff keeps retrying instead.
      exp_backoff_restart_delay: 200,
      min_uptime: '10s',
      // Caps a leak before it becomes an OOM on a 7.8GB box. It runs at
      // ~70MB proxying four apps, so this is generous headroom, not a
      // tight limit.
...
# Same smoke test redeploy.sh runs: "online" in pm2 means spawned, not
# able to serve. server-setup.md#activator-watchdog
echo "--> Waiting for ${PM2_ACTIVATOR_NAME} to answer its health check"
activator_healthy=false
for _ in $(seq 1 20); do
  if curl -sf --max-time 3 "http://127.0.0.1:${ACTIVATOR_PORT}/__activator/health" >/dev/null 2>&1; then
    activator_healthy=true; break
  fi
  sleep 1
done
if [[ "$activator_healthy" != true ]]; then
  echo ""
  echo "!! ${PM2_ACTIVATOR_NAME} did NOT answer its health check."
  echo "!! Scale-to-zero apps cannot be woken: a STOPPED app will 502."
  echo "!!   pm2 logs ${PM2_ACTIVATOR_NAME} --err --lines 40"
  echo "!! Provisioning continues; fix this before letting apps sleep:"
  echo "!!   sudo systemctl stop ${IDLE_WATCHER_UNIT:-embarko-idle-${APP_ENV}}.timer"
  echo ""
fi
...
# ---------------------------------------------------------------------------
# 9d. Activator watchdog.
#
#     pm2 restarts a process that CRASHES but cannot see one that is merely
#     wedged — still "online", serving nothing. That is the failure mode
#     nothing else here covers. server-setup.md#activator-watchdog
# ---------------------------------------------------------------------------
WATCHDOG_UNIT="embarko-activator-watchdog-${APP_ENV}"
WATCHDOG_SCRIPT="/usr/local/bin/${WATCHDOG_UNIT}.sh"
echo "--> Installing activator watchdog '${WATCHDOG_UNIT}' (every 2 min)"

cat > "${WATCHDOG_SCRIPT}" << EOF
#!/usr/bin/env bash
# Generated by server-setup.sh. Two probes before acting: a single failed
# check during a deploy restart is normal and must not cause a second
# restart on top of it.
set -uo pipefail
URL="http://127.0.0.1:${ACTIVATOR_PORT}/__activator/health"
for _ in 1 2; do
  curl -sf --max-time 5 "\${URL}" >/dev/null 2>&1 && exit 0
  sleep 5
done
echo "activator health check failed twice — restarting ${PM2_ACTIVATOR_NAME}"
sudo -u "${APP_USER}" PM2_HOME="${APP_HOME}/.pm2" pm2 restart "${PM2_ACTIVATOR_NAME}" --update-env
EOF
chmod +x "${WATCHDOG_SCRIPT}"

cat > "/etc/systemd/system/${WATCHDOG_UNIT}.service" << EOF
[Unit]
Description=Embarko activator watchdog (${APP_ENV})
After=network.target
...
[Unit]
Description=Check the Embarko activator every 2 minutes (${APP_ENV})

[Timer]
# OnActiveSec, not OnBootSec — see the idle watcher timer above.
OnActiveSec=1min
